...
import { TemperatureReading } from '../../../DTO/types/sensor';
...
export class TemperatureApi {
  public async getCurrentTemperature(deviceId: string): Promise<TemperatureReading> {
    const response = await httpClient.get<ApiResponse<TemperatureReading>>(
      `${API_ENDPOINTS.TEMPERATURE}/current/${deviceId}`
    );
  ...
  public async getTemperatureHistory(
    ...
    const response = await httpClient.get<ApiResponse<TemperatureReading[]>>(
      `${API_ENDPOINTS.TEMPERATURE}/history/${deviceId}?limit=${limit}`
    );
    return response.data;
  }

  public async getTemperatureStats(deviceId: string): Promise<TemperatureApiResponse> {
    const response = await httpClient.get<ApiResponse<TemperatureApiResponse>>(
      `${API_ENDPOINTS.TEMPERATURE}/stats/${deviceId}`
    );
    return response.data;
  }
}
